...
#   oc login https://api.cluster-thmg4.thmg4.sandbox4076.opentlc.com:6443   # for east
...
if [ -n "$TOKEN" ]; then
  ...
  if kubectl get secret "$SECRET_NAME" -n "$NS" &>/dev/null; then
    ...
    case "$CLUSTER" in
      east) SERVER_URL="https://api.cluster-thmg4.thmg4.sandbox4076.opentlc.com:6443" ;;
      west) SERVER_URL="https://api.cluster-2tjvj.2tjvj.sandbox5367.opentlc.com:6443" ;;
