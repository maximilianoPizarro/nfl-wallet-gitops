...
qa_03() {
  local ID="QA-03"
  test_header "$ID" "Egress (ESPN)" "api-bills and api-raiders can reach ESPN API"

  local east_bills_url="${SCHEME}://nfl-wallet-dev.${EAST_BASE}/api-bills"
  local east_raiders_url="${SCHEME}://nfl-wallet-dev.${EAST_BASE}/api-raiders"

  echo "  Testing ESPN egress via dev (no auth)..."

  local bills_code bills_body
  bills_code=$(curl_code "$east_bills_url")
  bills_body=$(curl_body "$east_bills_url" | head -c 500)

  local raiders_code raiders_body
  raiders_code=$(curl_code "$east_raiders_url")
  raiders_body=$(curl_body "$east_raiders_url" | head -c 500)

  echo "  api-bills  (${east_bills_url}): HTTP ${bills_code}"
  echo "  api-raiders (${east_raiders_url}): HTTP ${raiders_code}"

  local ok=true
  if [ "$bills_code" = "200" ]; then
    echo -e "    ${GREEN}✓${NC} api-bills returned 200"
  else
    echo -e "    ${RED}✗${NC} api-bills returned ${bills_code} (expected 200)"
    ok=false
  fi

  if [ "$raiders_code" = "200" ]; then
    echo -e "    ${GREEN}✓${NC} api-raiders returned 200"
  else
    echo -e "    ${RED}✗${NC} api-raiders returned ${raiders_code} (expected 200)"
    ok=false
  fi

  if $ok; then
    pass "ESPN egress working — APIs return 200 with data" "$ID"
  else
    fail "ESPN egress failed — check ztunnel egress and ServiceEntry" "$ID"
  fi
}
...
qa_05() {
  ...
  local url="${SCHEME}://nfl-wallet-test.${EAST_BASE}/api-customers"
  ...
  elif [ "$other_errors" -gt 0 ]; then
    fail "No 429 received but got ${other_errors} unexpected errors (check AuthPolicy/connectivity)" "$ID"
  else
    fail "All ${RATE_LIMIT_REQUESTS} requests returned 200 — RateLimitPolicy may not be configured" "$ID"
  fi
}
...
qa_06() {
  ...
  declare -a targets=(
    "test-east:${SCHEME}://nfl-wallet-test.${EAST_BASE}/api-bills"
    "test-west:${SCHEME}://nfl-wallet-test.${WEST_BASE}/api-customers"
    "prod-east:${SCHEME}://nfl-wallet-prod.${EAST_BASE}/api-bills"
  )
  ...
  for entry in "${targets[@]}"; do
    ...
    if [ "$code" = "403" ]; then
      ...
      echo -e "  ${RED}✗${NC} ${label}: HTTP ${code} (expected 403, got ${code})"
      all_ok=false
    fi
  done

  echo ""
  echo "  Verifying WITH valid key returns 200..."
  local with_key_code
  with_key_code=$(curl_code -H "X-Api-Key: ${API_KEY_BILLS}" "${SCHEME}://nfl-wallet-test.${EAST_BASE}/api-bills")
  if [ "$with_key_code" = "200" ]; then
    echo -e "  ${GREEN}✓${NC} test-east with key: HTTP 200"
  else
    echo -e "  ${RED}✗${NC} test-east with key: HTTP ${with_key_code} (expected 200)"
    all_ok=false
  fi
...
qa_07() {
  ...
  declare -a checks=(
    "east-customers:${SCHEME}://nfl-wallet-dev.${EAST_BASE}/api-customers"
    "east-bills:${SCHEME}://nfl-wallet-dev.${EAST_BASE}/api-bills"
    "east-raiders:${SCHEME}://nfl-wallet-dev.${EAST_BASE}/api-raiders"
    "west-customers:${SCHEME}://nfl-wallet-dev.${WEST_BASE}/api-customers"
    "west-bills:${SCHEME}://nfl-wallet-dev.${WEST_BASE}/api-bills"
    "west-raiders:${SCHEME}://nfl-wallet-dev.${WEST_BASE}/api-raiders"
  )
  ...
  echo ""
  echo "  Testing webapp on both clusters..."
  for cluster_label in east west; do
    local domain
    [ "$cluster_label" = "east" ] && domain="$EAST_BASE" || domain="$WEST_BASE"
    local webapp_url="${SCHEME}://webapp-nfl-wallet-dev.${domain}/"
    local code
    code=$(curl_code "$webapp_url")
    if [ "$code" = "200" ]; then
      echo -e "  ${GREEN}✓${NC} webapp-${cluster_label}: HTTP 200"
    else
      echo -e "  ${RED}✗${NC} webapp-${cluster_label}: HTTP ${code}"
      all_ok=false
    fi
  done
...
qa_08() {
  ...
  local grafana_url="${SCHEME}://grafana-nfl-wallet-service.${WEST_BASE}/"
  local promxy_url="${SCHEME}://promxy-acm-observability.${WEST_BASE}/"
  ...
  if require_oc; then
    ...
    local prom_query
    prom_query=$(curl_body "${SCHEME}://promxy-acm-observability.${WEST_BASE}/api/v1/query?query=istio_requests_total" 2>/dev/null | head -c 500 || true)
...
qa_10() {
  ...
  if [ "$total_429" -gt 0 ]; then
    ...
    echo -e "  ${YELLOW}!${NC} All requests returned 200 — limit may be higher than ${total} or not configured"
    pass "Load test complete — all ${total} requests succeeded (rate limit not triggered)" "$ID"
  else
    fail "Load test had ${total_err} errors" "$ID"
  fi
}
